...
func routeOpError(err error) error {
	if err == nil {
		return nil
	}
	...
	return &net.OpError{Op: "route", Net: "ip+net", Source: nil, Addr: nil, Err: err}
}

func netlinkInterfaces(ifindex int) ([]net.Interface, error) {
	ift, err := interfaceTable(ifindex)
	if err != nil {
		return nil, routeOpError(err)
	}
	return ift, nil
}

func netlinkInterfaceByIndex(index int) (*net.Interface, error) {
	if index <= 0 {
		return nil, routeOpError(errInvalidInterfaceIndex)
	}
	ift, err := netlinkInterfaces(index)
	if err != nil {
		return nil, err
	}
...
func netlinkInterfaceByName(name string) (*net.Interface, error) {
	if name == "" {
		return nil, routeOpError(errInvalidInterfaceName)
	}
	ift, err := netlinkInterfaces(0)
	if err != nil {
		return nil, err
	}
	for i := range ift {
	...
	return nil, routeOpError(errNoSuchInterface)
}

func netlinkInterfaceAddrs(ifi *net.Interface) ([]net.Addr, error) {
...
func Interfaces() ([]net.Interface, error) {
	netlink, nerr := netlinkInterfaces(0)
	std, serr := net.Interfaces()
	return chooseIfaces(IsForceAnet(), netlink, nerr, std, serr)
}
...
func InterfaceAddrs() ([]net.Addr, error) {
	netlink, nerr := netlinkInterfaceAddrs(nil)
	std, serr := net.InterfaceAddrs()
	return chooseAddrs(IsForceAnet(), netlink, nerr, std, serr)
}
...
func InterfaceByIndex(index int) (*net.Interface, error) {
	netlink, nerr := netlinkInterfaceByIndex(index)
	std, serr := net.InterfaceByIndex(index)
	return chooseIface(IsForceAnet(), netlink, nerr, std, serr)
}

// InterfaceByName returns the interface specified by name.
func InterfaceByName(name string) (*net.Interface, error) {
	netlink, nerr := netlinkInterfaceByName(name)
	std, serr := net.InterfaceByName(name)
	return chooseIface(IsForceAnet(), netlink, nerr, std, serr)
}
...
func InterfaceAddrsByInterface(ifi *net.Interface) ([]net.Addr, error) {
	if ifi == nil {
		return nil, routeOpError(errInvalidInterface)
	}
	netlink, nerr := netlinkInterfaceAddrs(ifi)
	std, serr := ifi.Addrs()
	return chooseAddrs(IsForceAnet(), netlink, nerr, std, serr)
}
