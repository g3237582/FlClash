...
func init() {
	...
	if runtime.GOOS == "android" { // Android SDK 30 no longer permits Go's net.Interfaces to work (Issue 2293)
		netmon.RegisterInterfaceGetter(func() ([]netmon.Interface, error) {
			...
			var nif []netmon.Interface
			for _, iff := range ifaces {
				addrs, addrErr := anet.InterfaceAddrsByInterface(&iff)
				if addrErr != nil {
					log.Warnln("[Tailscale] anet.InterfaceAddrsByInterface(%v) failed: %v", iff.Name, addrErr)
					continue
				}
